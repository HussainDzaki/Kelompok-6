...
while microwave_nyala == True :
    ...
    if voice_command_on == "iya": #input microwave berbasis suara \
        ...
        input_invalid = True
        back_state = False
        while input_invalid == True or back_state == True :
            input_invalid = False
            ...
            if input_preset == 0 or input_preset == no_scan :
                ...
                while input_invalid == True :
                    ...
                    if save_as_preset == "iya" :
                        ...
                        Saved_Presets[current_taken_slot][2] = level_value
                        Saved_Presets[current_taken_slot][3] = heat_cap_value
    ...
    read_mass = int(input("Massa yang terbaca (gram): ")) #Membaca massa menggunakan neraca internal microwave
    ...
    while waktu >= 0 and timer_nyala : 
        ...
        if asap_terdeteksi ==  False :
            if ((wattage_microwave * level_value) - fan_efficiency)/(heat_cap_value/100 * read_mass) < 0.5 :
                temp_predict += ((wattage_microwave * level_value) - fan_efficiency)/(heat_cap_value/100 * read_mass)
            else :
                temp_predict += 0.5
        else : 
            temp_predict -= (fan_efficiency)/(heat_cap_value/100 * read_mass)

        
        if temp_predict > heat_cap_value * 0.5953125 and asap_terdeteksi == False: #Prediksi kapan terjadinya asap
            asap_terdeteksi = True
        elif temp_predict < 30 and asap_terdeteksi == True :
            print("Makanan aman untuk dikeluarkan.")
            asap_terdeteksi = False
            input_invalid = True
            while input_invalid :
                input_invalid = False
                lanjut_operasi = input("Apakah Anda ingin melanjutkan operasi microwave? (iya atau tidak): ")
        ...
        if asap_terdeteksi == False : 
            ...
            if flash == True :
                tampilan_waktu = "ASAP TERDETEKSI"
                flash = False
            else :
                tampilan_waktu = "               "
                flash = True
